...
void setup_I2C(i2c_inst_t *I2C_PORT, uint I2C_SDA, uint I2C_SCL)
{
...
void setup_ssd1306(ssd1306_t *ssd, uint8_t endereco, i2c_inst_t *I2C_PORT)
{
...
void ssd1306_draw_char(ssd1306_t *ssd, char c, uint8_t x, uint8_t y)
{
    ...
    if (c >= '0' && c <= '9') {
        ...
        switch (c) {
            case ' ': index = 0; break;       // Espaço
            case '!': index = (63) * 8; break; // Exclamação (!)
            case ':': index = (64) * 8; break; // Dois pontos (:)
            case '.': index = (65) * 8; break; // Ponto (.)
            case '/': index = (66) * 8; break; // Barra (/)
            case ',': index = (67) * 8; break; // Vírgula (,)
            case '-': index = (68) * 8; break; // Hífen (-)
            case '?': index = (69) * 8; break; // Interrogação (?)
            case '*': index = (70) * 8; break; // asterisco (*)
            case '<': index = (71) * 8; break; // Menor que (<)
            case '^': index = (72) * 8; break; // Circunflexo (^)
            default: return; // Ignorar caracteres não suportados
        }
    }
...
void draw_filled_square(ssd1306_t *ssd, uint8_t x, uint8_t y) {
  for (uint8_t i = 0; i < 8; i++) {
      for (uint8_t j = 0; j < 8; j++) {
          ssd1306_pixel(ssd, x + j, y + i, true); // Desenha um pixel
      }
  }
